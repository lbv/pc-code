...
const int MaxRem = 2000;

int memo[1 << MAXLEN][MaxRem + 1];
...
int memoize(u32 b, int r, int res)
{
	if (r <= MaxRem) memo[b][r] = res;
	return res;
}
...
{
	if (r <= MaxRem && memo[b][r] >= 0) return memo[b][r];

	if (n == 1)
		return memoize(b, r, (s[Ctz(b)] % d == r ? 1 : 0));
